...
export function AddCardModal({ isOpen, onClose, onCardAdded }: AddCardModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.type.trim()) {
            setError('Le type de la carte est requis');
            setLoading(false);
            return;
        }
        ...
        try {
            ...
            if (!response.ok) {
                const errorData = await response.json().catch(() => ({ message: 'Erreur lors de l\'ajout de la carte' }));
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                <div className="p-6">
                    <div className="flex justify-between items-start mb-6">
                        <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Ajouter une nouvelle carte</h2>
                        <button
                            onClick={handleClose}
                            disabled={loading}
